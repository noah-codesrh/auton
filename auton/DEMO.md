...
| Service | URL |
|---------|-----|
| Frontend | https://www.autonairh.xyz (or `pnpm dev` → localhost:5173) |
| Backend API | https://api.autonairh.xyz |
| Health | https://api.autonairh.xyz/health |
| DB health | https://api.autonairh.xyz/health/db |
| On-chain config | https://api.autonairh.xyz/api/v1/config |
...
1. All 6 required env vars set (see `backend/RAILWAY.md`)
2. `CORS_ORIGINS` includes your frontend URL:
   ```
   https://autonairh.xyz,https://www.autonairh.xyz,https://app.autonairh.xyz
   ```
...
Enable **EVM wallets** for Robinhood Chain.
...
```bash
curl https://api.autonairh.xyz/api/v1/gateway/v1/chat/completions \
...
```bash
# Backend up?
curl -s https://api.autonairh.xyz/health | jq .

# Supabase connected?
curl -s https://api.autonairh.xyz/health/db | jq .

# Your mint + vault from Railway?
curl -s https://api.autonairh.xyz/api/v1/config | jq .

# Auth nonce works?
curl -s "https://api.autonairh.xyz/api/v1/auth/nonce/YOUR_WALLET" | jq .
```
